...
public class ETHWalletUtils {
     ...
     * @return
     */
    public static ETHWallet generateMnemonic(String jaxx_type,String walletName, String pwd,List<String> list) {
        //String[] pathArray = ETH_JAXX_TYPE.split("/");
...
//            ToastUtils.showToast(R.string.load_wallet_by_official_wallet_keystore_input_tip);
     ...
     * @return
     */
    public static ETHWallet loadWalletByPrivateKey(String wallerName,String privateKey, String pwd) {
        ECKeyPair ecKeyPair = ECKeyPair.create(Numeric.toBigInt(privateKey));
    ...
    public static ETHWallet modifyPassword(long walletId, String walletName, String oldPassword, String newPassword) {
        ...
        ETHWallet ethWallet =WalletDaoUtils.loadSingle(walletId);
        Credentials credentials = null;
    ...
    public static void deleteWallet(long walletId) {
       ...
       /* ETHWallet ethWallet =WalletDaoUtils.loadSingle(walletId);
        ...
        SQLite.delete().from(ETHWallet.class).where(ETHWallet_Table.id.eq(walletId)).execute();
    }
...
//            System.out.println("删除单个文件失败：" + fileName + "不存在！");
    ...
    public static String getLastType(){
        ...
        List<ETHWallet> byCreate = WalletDaoUtils.loadAllByCreate();
        if(byCreate!=null && byCreate.size()>0) {
            ETHWallet ethWallet = byCreate.get(0);
